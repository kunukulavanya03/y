...
def get_all_users(db=Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    return JSONResponse(content={'users': [user_schema.json() for user_schema in users], 'total': len(users)})

@app.post('/api/users')
def create_user(user_schema: UserSchema, db=Depends(get_db), token: str = Depends(oauth2_scheme)):
...
def get_user_by_id(user_id: int, db=Depends(get_db), token: str = Depends(oauth2_scheme)):
...
def update_user(user_id: int, user_schema: UserSchema, db=Depends(get_db), token: str = Depends(oauth2_scheme)):
    user = get_user(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail='User not found')
...
def register(user_schema: UserSchema):
    user = User(name=user_schema.name, email=user_schema.email, password=get_password_hash(user_schema.password))
    db = get_db()
    db.add(user)
    db.commit()
...
@app.put('/api/profile')
def update_profile(user_schema: UserSchema, token: str = Depends(oauth2_scheme)):
    user_id = validate_token(token)
    user = get_user_by_id(user_id)
    user.name = user_schema.name
    user.email = user_schema.email
...
@app.post('/api/data')
def create_data(data_schema: DataSchema, token: str = Depends(oauth2_scheme)):
    user_id = validate_token(token)
...
@app.get('/api/data')
def get_all_data(token: str = Depends(oauth2_scheme)):
    user_id = validate_token(token)
    data = get_data_by_user_id(user_id)
    return JSONResponse(content={'data': [data_schema.json() for data_schema in data]})

@app.get('/api/data/{data_id}')
def get_data_by_id(data_id: int, token: str = Depends(oauth2_scheme)):
...
def update_data(data_id: int, data_schema: DataSchema, token: str = Depends(oauth2_scheme)):
    data = get_data(data_id)
    if not data:
        raise HTTPException(status_code=404, detail='Data not found')
...
def delete_data(data_id: int, token: str = Depends(oauth2_scheme)):
    data = get_data(data_id)
    if not data:
        raise HTTPException(status_code=404, detail='Data not found')
    db = get_db()
